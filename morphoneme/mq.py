...
Inflectional suffixes are loaded from inf_suffixes.txt (alongside morphoneme.db).
"""
...
_PKG_DB_PATH = Path(__file__).parent / "morphoneme.db"
_USER_HOME_DB = Path.home() / ".morphoneme" / "morphoneme.db"
...
def ensure_db_exists(db_path: Path) -> None:
    ...
    url = "https://github.com/connoryang331/morphoneme/releases/download/v0.1.0/morphoneme.db.zip"
    zip_tmp = db_path.parent / "morphoneme.db.zip"
...
class MQ:
    ...
    def search(
        ...
        Args:
          ...
          fq          — frequency filter: comma-separated values from "high" | "medium" | "low" | "rare" | "zero"
        """
        if exact:
            match seg:
                case "umlabeller":
    ...
    def _get_fq_condition(fq: str | None, prefix: str = "") -> tuple[str, list]:
        ...
        # Split by comma to support multiple selection
        levels = [l.strip().lower() for l in fq.split(",") if l.strip()]
        if not levels:
            return "", []
        ...
        for level in levels:
            if level == "high":
                clauses.append(f"{col} >= 5.0")
            elif level == "medium":
                clauses.append(f"({col} >= 1.0 AND {col} < 5.0)")
            elif level == "low":
                clauses.append(f"({col} >= 0.1 AND {col} < 1.0)")
            elif level == "rare":
                clauses.append(f"({col} > 0.0 AND {col} < 0.1)")
            elif level == "zero":
                clauses.append(f"({col} == 0.0 OR {col} IS NULL)")
            else:
                raise ValueError("fq must contain comma-separated values from 'high', 'medium', 'low', 'rare', 'zero'")
                
        sql_clause = " OR ".join(clauses)
        return f"({sql_clause})", []
