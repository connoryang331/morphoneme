...
为了保持安装包的轻量化，SQLite 数据库文件（`morph_query.db`，约 50MB）**并未**打包进 PyPI 发布包中。

当您首次实例化 `MQ` 类或在终端运行命令行工具时：
1. 程序会优先寻找包目录下是否已有打包好的数据库（用于本地开发或手动放置）。
2. 如果未找到，程序会检查 `~/.morph_query/morph_query.db` 路径。
3. 如果依然缺失，程序将**自动从 GitHub Releases 下载**预编译压缩的数据库 zip 包，并解压至 `~/.morph_query/` 目录下。
...
| `--limit=N` | 限制返回的结果数量 |
| `--fq=VAL` | 按词频三挡过滤结果：`high` (>=5.0), `medium` (>=1.0 且 <5.0), `low` (<1.0)。**注**：无频数数据（`NULL`）的单词在任一档位中都**不会**被过滤掉。 |
...
results = mq.search("ion")                      # 模糊查询所有字段
results = mq.words_with_prefix("un")            # 查询指定前缀
results = mq.words_with_suffix("ing")           # 查询指定后缀
results = mq.words_with_root("believe")         # 查询指定词根
results = mq.words_with_deri("able")            # 查询指定派生后缀
results = mq.words_with_inf("ed")               # 查询指定屈折后缀

# 形态学分析
seg = mq.morph_seg("unbelievable")   # → "un-believe-able"
count = mq.morph_count("running")    # → 2
morph = mq.word_morph("cats")        # → 结构字典 (dict)
...
mq.batch_words("words.txt", mode="morph", fmt="csv")
mq.batch_words("words.txt", mode="morph:ai", fmt="json")  # 附加 AI 规则校验
```
...
本工具的屈折后缀列表存放在 `morph_query/inf_suffixes.txt` 中，每行一个后缀（支持带 `-` 前缀）。默认配置包括：
...
在 `morph-query` 中，命令行中的 `prefix`、`suffix` 和 `root` 指令（以及 Python API 对应的同名方法）在概念上是通用 `search` 命令的**语义化别名**。在底层，它们现在会通过关系表 `word_morphemes` 进行针对性索引查询，以提供极速、精准的结构定位。

设置别名提供了更符合人类直觉的交互入口（如 `mq prefix un` 比模糊搜索更精确易读）。

### 与 Datamuse, Webster, 和 OneLook 的区别

使用 `morph-query` 进行查询与使用在线的 **Datamuse API**、**Merriam-Webster** 或 **OneLook** 进行通配符或子串搜索有着本质的区别：
...
* **morph-query:** 查询的是数据库中的**词素切分标注数据列**。
  ...
  * *极大降噪:* 虽然标注列检索也不能确保 100% 正确（因为底层的学术数据集是由不同研究团队人工及算法整理，存在微小的标注不一致），但它和拼写匹配有本质区别：
    ...
    * *morph-query 的噪声* 仅属于学术标注数据集的小瑕疵，相对而言就像是**窗外树梢上小鸟的清脆叫声**。

#### 2. 本地数据库 vs. 外部 Web API
* **Datamuse / Webster / OneLook:** 都是在线服务。调用它们需要发起 HTTP 网络请求，这会引入网络延迟、受到频率限制、依赖网络环境，并且可能需要注册 API Key。
* **morph-query:** 完全**在本地运行**，自带预编译好的 SQLite 数据库。它不需要联网，查询在不到一毫秒内即可执行完毕，非常适合十万级词单的批量计算。

#### 3. 结构化形态学输出
* **Datamuse / Webster / OneLook:** 仅返回释义、同义词或单词列表，无法返回单词的内在词法构成。
* **morph-query:** 提供完整的单词形态结构剖析。能够拆分并分类前缀、词根、派生后缀和屈折后缀，使用户轻松获取词元并分析结构。

## 项目结构

```
morph_query/
├── morph_query/                 # Python 源码包 (发布至 PyPI)
│   ├── __init__.py
│   ├── __main__.py              # CLI 命令行入口
│   ├── mq.py                    # 核心 MQ 类
│   ├── morph_query.db           # SQLite 本地数据库 (自动下载或本地生成)
...
如果您有任何功能建议、Bug 反馈或咨询，欢迎在 [GitHub Issues](https://github.com/connoryang331/morph-query/issues) 页面提交。
